	...
	function popup() {
		...
		$(function() {
			$('.mailinfo').on('click', function(event) {
				...
				var no = $(this).data("no");
				$.ajax({
					  url: '${pageContext.request.contextPath}/onemailselect?lett_no=' + no ,
					  success: function(result) {
				modal.find('.id').text(result.mem_id)
				modal.find('.mid').text(result.lett_sid)
				modal.find('.title').text(result.lett_title)
				modal.find('.content').text(result.lett_content)
						modal.modal('show');
					}
			})
			})
